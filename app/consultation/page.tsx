...
function ConsultationContent() {
  ...
  const handleStartCall = () => {
    setHasStarted(true);
    setIsRecording(true);
    // Initialize Web Speech API here later
  };
      ...
      {!hasStarted && (
        <div className="absolute inset-0 z-[100] bg-slate-50/90 backdrop-blur-xl flex flex-col items-center justify-center">
          ...
          <Button 
            ...
            className="bg-slate-900 hover:bg-blue-600 text-white px-10 py-7 rounded-2xl text-lg font-bold shadow-xl transition-all hover:scale-105 flex gap-3"
          >
      ...
      <header className="px-8 py-4 flex justify-between items-center bg-white border-b border-slate-100 sticky top-0 z-50">
        ...
        <div className="flex items-center gap-4">
          ...
          <button 
            ...
            className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-slate-100 text-slate-600 text-xs font-bold hover:bg-slate-200 transition-colors uppercase"
          >
            <Globe size={14} />
        ...
        <div className="lg:w-1/3 flex flex-col items-center justify-center space-y-6">
          ...
          <div className="flex items-end gap-1.5 h-10">
            {[...Array(10)].map((_, i) => (
              <div 
                key={i} 
                className={`w-2 bg-blue-500/20 rounded-full transition-all duration-300 ${isRecording ? 'animate-wave' : 'h-2'}`}
                style={{ 
                  animationDelay: `${i * 0.1}s`, 
                  height: isRecording ? '100%' : '20%',
                  backgroundColor: isRecording ? '#3b82f6' : '#e2e8f0'
                }}
              />
            ))}
          </div>
          ...
          <div className="p-10 bg-white border-t border-slate-50 flex justify-center items-center gap-8">
            <Button 
              ...
              className={`w-20 h-20 rounded-full shadow-2xl transition-all duration-300 ${
                isRecording ? "bg-blue-600 hover:bg-blue-700 scale-110" : "bg-red-500 text-slate-400 hover:bg-red-600"
              }`}
            >
            ...
            <Button 
              variant="outline" 
              className="h-16 px-10 rounded-2xl border-slate-200 text-slate-600 font-bold hover:bg-red-50 hover:text-red-600 hover:border-red-100 transition-all flex gap-3 shadow-sm"
              onClick={endCall}
            >
